...
$runId = time();
$adminEmail = 'cnc_admin_' . $runId . '@example.com';
$pilgrimEmail = 'cnc_pil_' . $runId . '@example.com';
...
echo "=== Testing Booking Cancellation Request ===\n\n";
...
$pkg = callApi($baseUrl . '/packages', 'POST', ['package_name' => 'Cnc Pkg', 'price' => 1000, 'duration_days' => 5, 'is_active' => true], $adminToken);
$pkgId = $pkg['body']['package']['package_id'];

$trip = callApi($baseUrl . '/trips', 'POST', ['package_id' => $pkgId, 'trip_name' => 'Cnc Trip', 'start_date' => date('Y-m-d'), 'end_date' => date('Y-m-d', strtotime('+5 days')), 'status' => 'PLANNED'], $adminToken);
...
echo "3. Requesting Cancellation...\n";
$res = callApi($baseUrl . "/bookings/$bkId/request-cancellation", 'POST', ['reason' => 'Emergency'], $pilgrimToken);

if ($res['code'] === 201) {
    echo "SUCCESS: Cancellation request submitted.\n";
    $m = $res['body']['modification'];
    echo "ID: " . $m['modification_id'] . "\n";
    echo "Status: " . $m['status'] . "\n";
    echo "Type: " . $m['request_type'] . "\n";
    echo "Reason: " . $m['request_data']['reason'] . "\n";

    if ($m['status'] === 'PENDING' && $m['request_type'] === 'CANCELLATION') {
